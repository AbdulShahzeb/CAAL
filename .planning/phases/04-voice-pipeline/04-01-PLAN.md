...
must_haves:
  truths:
    ...
    - "French prompt instructs the agent to respond in French"
    - "English prompt is identical to current prompt (no regression)"
    - "Custom prompts still take priority over per-language defaults"
    - "Backward compatibility: if prompt/en/ directory doesn't exist, falls back to prompt/default.md"
  artifacts:
...
Purpose: The agent needs localized system prompts so it responds in the user's configured language, and date/time context in the prompt must also be formatted in that language.
...
3. Create `prompt/fr/default.md` -- a full French translation of the English prompt. Key translation notes:
   ...
   - Add explicit instruction: "Reponds toujours en francais." (use accent: fran\u00e7ais)
...
2. Verify prompt loading:
   ...
   print('EN prompt starts with:', en[:50])
   print('FR prompt starts with:', fr[:50])
   assert 'ACTION-ORIENTED' in en
   assert 'ACTION' in fr or 'action' in fr.lower()
   print('OK')
   "
   ```
...
- prompt/fr/default.md exists with French prompt
...
4. Backward compatibility: `load_prompt_content("default")` still works (falls back correctly)
5. `prompt/default.md` still exists (not deleted)
</verification>
