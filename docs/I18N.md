...
**a) TTS voice mapping** — Add a Piper voice in `voice_agent.py`:
...
| 3 | Piper voice mapping | `voice_agent.py` → `PIPER_VOICE_MAP` |
...
|-----------|------|---------|
| Settings  | `src/caal/settings.py` | Language storage, prompt/greetings loading |
| Voice     | `voice_agent.py` | Voice map, pipeline config |
